...
class VolumenVentana(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.label_fuente_audio_selec = QLabel(f"Index: 0 \ {self.appsl[0]}") #inicializa el label en el primer canal
        self.label_fuente_audio_selec.setAlignment(Qt.AlignHCenter)
    ...
    def crear_panel_de_audio(self, appfa, color):
        ...
        self.fuente_audio[appfa].setText(f"Index: {str(appfa)}") # \ {self.appsl[appfa]}")
        self.fuente_audio[appfa].setAlignment(Qt.AlignHCenter)
    ...
    def cambiar_app_select(self, index):
        """ selecciona al canal la fuente de audío índicada por el combobox
            además y como si fuera poco implementa una verificación de estatus
            de los botones de silencio de los canales individuales, ya que si 
            se selecciona con el combobox o lista desplegable una fuente de audio
            que en su canal se encuentra silenciada esto debe replicarse en el canal
            de fuentes de audio seleccionables como es lógico pensar, o al menos
            eso diria Spok, quien evaluaría una depuración de código general de 
            todo el software disponible en el planeta tierra...

            ...por las dudas...

            ...yo digo nomás, ustedes hagan lo que quieran.
            para vos Spok:
            
            indice en cambiar_app_select:  1
            el boton silencio del canal/indice seleccionado está:  False
            indice en cambiar_app_select:  0
            el boton silencio del canal/indice seleccionado está:  True
            Traceback (most recent call last):
                File "/home/alan/Visual Studio Python/volume/volume448.py", line 557, in cambiar_app_select
                    self.mutefun() 
            TypeError: mutefun() missing 1 required positional argument: 'cucu'

            ...si, me da un poco de verguenza, pero estoy fumado y tome vino
            del dulce barato, con manaos de pomelo y la musica que estoy escuchando
            para probar el control... jajajaja me pone loco loco loco si me
            fumo seis satoshis me pongo nakamoto
        ...
        if index < len(self.apps):
            print("indice en cambiar_app_select: ", index)
            print("el boton silencio del canal/indice seleccionado está: ", self.btn_mute_canal[index].isChecked())
            if self.btn_mute_canal[index].isChecked():
                self.btn_mute_audio.setChecked(True)
                self.mutefun() 
            ...
            self.label_fuente_audio_selec.setText(f"Index: {index} \ {self.appsl[index]}")
            self.label_volumen.setText(f"Volumen: {self.volumen}")
